 ...
 *   - src/components/battle/spriteImageSizes.ts 不硬編碼資料，僅「彙總」各資料夾
 *     的快取（import 檔名快取 → 補回資料夾前綴成 URL 鍵），由本腳本自動產生。
 *     The generated spriteImageSizes.ts holds NO hardcoded data; it only
 *     aggregates the per-folder caches (import + prefix back to URL keys).
 *
 * 用途 / Purpose:
...
/** 產出的彙總 TS（不含資料，僅 import + spread） / Generated aggregator TS (no data, only import+spread) */
const INDEX_TS = resolve(process.cwd(), 'src/components/battle/spriteImageSizes.ts');
