...
export function guardActive(guardChar: Character): boolean {
	const kind = guardChar.behavior?.guard ?? 'always';
	switch (kind) {
		case 'always': return true;
		case 'never': return false;
		case 'life25': return guardChar.hpPercent() <= 25;
		case 'life50': return guardChar.hpPercent() <= 50;
		case 'life75': return guardChar.hpPercent() <= 75;
		case 'prob25': return guardChar.rng ? guardChar.rng.randInt(0, 99) < 25 : false;
		case 'prob50': return guardChar.rng ? guardChar.rng.randInt(0, 99) < 50 : false;
		case 'prob75': return guardChar.rng ? guardChar.rng.randInt(0, 99) < 75 : false;
		default: return true;
	}
}
...
export function Defending(team: BattleTeam, target: Character, skill: ISkillDef): Character | null {
	if (skill.target?.[0] === 'all') return null;
