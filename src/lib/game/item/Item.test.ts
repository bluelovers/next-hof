...
describe('Equip system (7.2)', () => {
	it('dh main-hand unequips off-hand; handle overflow fails', () => {
		...
		r.addItem({ no: 1000, name: 'ShortSword', type: 'Sword', type2: 'WEAPON', atk: [10, 0], def: [0, 0, 0, 0], handle: 1, need: {} });
		r.addItem({ no: 3000, name: 'WoodShield', type: 'Shield', type2: 'ARMOR', atk: [0, 0], def: [5, 5, 0, 0], handle: 1, need: {} });
		r.addItem({ no: 9000, name: 'BigSword', type: 'TwoHandSword', type2: 'WEAPON', atk: [50, 0], def: [0, 0, 0, 0], dh: true, handle: 3, need: {} });
		r.addItem({ no: 9999, name: 'Heavy', type: 'Armor', type2: 'ARMOR', atk: [0, 0], def: [0, 0, 0, 0], handle: 10, need: {} });
		...
		// 雙手互斥：裝備 dh 主手 → 卸下副手
		const [fail, removed] = setEquip(char, r, 'main_hand', 9000);
		...
		const char2 = new Character({
			no: 2, name: 'c2', types: ['char'], level: 1,
			str: 10, int: 10, dex: 4, spd: 10, luk: 10, maxhp: 300, maxsp: 50,
		});
		const [fail2] = setEquip(char2, r, 'main_hand', 9999);
